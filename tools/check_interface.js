async page => {
  ...
  const labels = ["To-do list", "Messages", "Calls", "Close phone"];
  const buttons = [];
  ...
  for (const label of labels.slice(0, 3)) await page.locator("#dialog-hotspots").getByRole("button", {name:label,exact:true}).click();
